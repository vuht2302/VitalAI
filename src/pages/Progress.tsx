  ...
  return (
    <div className="space-y-8 pb-12">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-display font-bold text-zinc-900">Progress & Analytics</h1>
          <p className="text-zinc-500 font-medium mt-1">Deep dive into your fitness journey data.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          <section className="bg-white p-8 rounded-[2rem] border border-zinc-100 shadow-sm">
            <div className="flex items-center justify-between mb-8">
              <div>
                <h2 className="text-xl font-display font-bold text-zinc-900">Weight Tracker</h2>
                <p className="text-sm text-zinc-500 font-medium">Last 30 days trend</p>
              </div>
              <div className="flex items-center gap-4">
                <div className="text-right">
                  <p className="text-2xl font-display font-bold text-zinc-900">78.5 kg</p>
          ...
          <section className="bg-white p-8 rounded-[2rem] border border-zinc-100 shadow-sm">
            <div className="flex items-center justify-between mb-8">
              <div>
                <h2 className="text-xl font-display font-bold text-zinc-900">Calorie Consistency</h2>
                <p className="text-sm text-zinc-500 font-medium">Daily intake vs. target goal</p>
              </div>
                ...
                <BarChart data={calorieData}>
                  ...
                  <Bar dataKey="calories" radius={[6, 6, 0, 0]}>
                    {calorieData.map((entry, index) => (
                      <Cell key={`cell-${index}`} fill={entry.calories > entry.target ? '#ef4444' : '#22c55e'} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
        ...
        <div className="space-y-8">
          <section className="bg-white p-8 rounded-[2rem] border border-zinc-100 shadow-sm">
            <h2 className="text-lg font-display font-bold text-zinc-900 mb-6">Body Composition</h2>
          ...
          <section className="bg-zinc-900 p-8 rounded-[2rem] text-white relative overflow-hidden">
            <div className="relative z-10">
              ...
              <h3 className="text-xl font-display font-bold mb-4">Milestone Reached!</h3>
              <p className="text-zinc-400 text-sm leading-relaxed mb-6">
                You've maintained a 5-day calorie consistency streak. This is 20% better than your last month's performance.
              </p>
              <div className="flex items-center gap-4">
          ...
          <section className="bg-white p-8 rounded-[2rem] border border-zinc-100 shadow-sm">
            <h3 className="text-lg font-display font-bold text-zinc-900 mb-6">Historical Data</h3>
            <div className="space-y-4">
              {[
                ...
                <div key={item.label} className="flex items-center justify-between p-4 bg-zinc-50 rounded-2xl">
                  <span className="text-sm font-medium text-zinc-600">{item.label}</span>
                  <div className="text-right">
                    <p className="text-sm font-bold text-zinc-900">{item.value}</p>
                    <p className={`text-[10px] font-bold ${item.trend.startsWith('+') ? 'text-green-600' : 'text-red-600'}`}>{item.trend}</p>
                  </div>
                </div>
              ))}
            </div>
