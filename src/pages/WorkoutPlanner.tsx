  ...
  return (
    <div className="space-y-8 pb-12">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-display font-bold text-zinc-900">Workout Planner</h1>
          <p className="text-zinc-500 font-medium mt-1">Plan your week and track your performance.</p>
        </div>
      ...
      {viewMode === 'flow' ? (
        <section className="bg-white p-8 rounded-[2rem] border border-zinc-100 shadow-sm min-h-[400px] relative overflow-hidden">
          <h2 className="text-xl font-display font-bold text-zinc-900 mb-8">Workout Sequence Flow</h2>
              ...
              <React.Fragment key={node.id}>
                <motion.div 
                  ...
                  className={`w-32 h-32 rounded-3xl flex flex-col items-center justify-center text-center p-4 shadow-lg relative z-10 ${
                    node.type === 'start' ? 'bg-brand-600 text-white' : 
                    node.type === 'end' ? 'bg-zinc-900 text-white' : 
          ...
          <div className="mt-12 p-6 bg-zinc-50 rounded-2xl border border-zinc-100">
            <p className="text-sm font-medium text-zinc-500 italic">
              * This flow builder allows you to visualize the sequence of your exercises. Drag and drop to reorder (Coming soon).
            </p>
          </div>
        </section>
      ) : (
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            <h2 className="text-xl font-display font-bold text-zinc-900">May 2024</h2>
          ...
          <div className="flex bg-zinc-100 p-1 rounded-xl">
            <button className="px-4 py-1.5 bg-white rounded-lg text-sm font-bold text-zinc-900 shadow-sm">Week</button>
            <button className="px-4 py-1.5 text-sm font-bold text-zinc-500 hover:text-zinc-900 transition-colors">Month</button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-7 gap-4">
          {days.map((day, idx) => (
            <div key={day} className="text-center">
              <p className="text-xs font-bold text-zinc-400 uppercase tracking-widest mb-4">{day}</p>
              <button 
              ...
              >
                ...
                {dates[idx] % 3 === 0 && <div className={`w-1.5 h-1.5 rounded-full ${dates[idx] === 24 ? 'bg-white' : 'bg-brand-500'}`}></div>}
              </button>
            </div>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          <div className="flex items-center justify-between">
            <h3 className="text-xl font-display font-bold text-zinc-900">Today's Schedule</h3>
            <span className="text-sm font-bold text-zinc-400">3 Workouts Planned</span>
          </div>
            ...
            {mockWorkouts.map((workout, idx) => (
              ...
              >
                <div className="flex items-center gap-6">
                  <div className={`w-16 h-16 rounded-2xl flex items-center justify-center transition-colors ${workout.completed ? 'bg-green-50 text-green-600' : 'bg-brand-50 text-brand-600 group-hover:bg-brand-600 group-hover:text-white'}`}>
                    {workout.completed ? <CheckCircle2 size={32} /> : <Dumbbell size={32} />}
                  </div>
                  
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <h4 className={`text-lg font-bold ${workout.completed ? 'text-zinc-400 line-through' : 'text-zinc-900'}`}>
                        {workout.title}
                      </h4>
                      {workout.completed && <span className="text-[10px] font-bold uppercase tracking-wider bg-green-100 text-green-700 px-2 py-0.5 rounded-full">Completed</span>}
                    </div>
                    <div className="flex items-center gap-4 text-sm font-medium text-zinc-500">
                      <span className="flex items-center gap-1.5"><Clock size={14} /> {workout.duration} mins</span>
                      <span className="flex items-center gap-1.5"><Flame size={14} /> {workout.calories} kcal</span>
                      <span className="flex items-center gap-1.5"><CalendarIcon size={14} /> 09:30 AM</span>
                    </div>
                  </div>
        ...
        <div className="space-y-8">
          <section className="bg-zinc-900 p-8 rounded-[2rem] text-white relative overflow-hidden">
            <div className="relative z-10">
              <h3 className="text-xl font-display font-bold mb-6">AI Recommendations</h3>
              <div className="space-y-6">
                <div className="p-4 bg-white/10 rounded-2xl backdrop-blur-sm border border-white/10">
                  <p className="text-xs font-bold text-brand-400 uppercase tracking-widest mb-2">Next Session</p>
                  <h4 className="font-bold mb-1">HIIT Cardio Blast</h4>
                  <p className="text-sm text-zinc-400 mb-4">Focus on explosive movements to boost metabolism.</p>
                  <button className="w-full py-2 bg-brand-500 text-white rounded-xl text-sm font-bold hover:bg-brand-600 transition-colors">
                    Add to Schedule
                  </button>
                </div>
                <div className="p-4 bg-white/10 rounded-2xl backdrop-blur-sm border border-white/10">
                  <p className="text-xs font-bold text-brand-400 uppercase tracking-widest mb-2">Recovery Tip</p>
                  <p className="text-sm text-zinc-300 leading-relaxed">
                    You've had 3 heavy lifting days. Consider a light yoga session or active recovery tomorrow.
                  </p>
                </div>
              </div>
            </div>
          ...
          <section className="bg-white p-8 rounded-[2rem] border border-zinc-100 shadow-sm">
            <h3 className="text-lg font-display font-bold text-zinc-900 mb-6">Workout Categories</h3>
            <div className="grid grid-cols-2 gap-4">
              {['Strength', 'Cardio', 'Yoga', 'HIIT', 'Pilates', 'Boxing'].map((cat) => (
                <button key={cat} className="p-4 bg-zinc-50 rounded-2xl text-sm font-bold text-zinc-600 hover:bg-brand-50 hover:text-brand-600 transition-all border border-transparent hover:border-brand-100">
                  {cat}
                </button>
              ))}
            </div>
